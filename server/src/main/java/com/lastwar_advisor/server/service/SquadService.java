...
public class SquadService {
    ...
    public Squad saveSquad(Long playerId, Integer squadNumber, List<SquadSlotRequest> squad) {
        ...
        for (SquadSlotRequest slotReq : squad) {
            System.out.println("Processing slot: " + slotReq.getPosition() + " " + slotReq.getSlotIndex() + " heroId: "
                    + slotReq.getHeroId());
            SquadSlot slot = repo_ss
                    .findBySquadAndPositionAndSlotIndex(finalSquad, slotReq.getPosition(), slotReq.getSlotIndex())
